...
# and `pip install datathon2022` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "datathon2022", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n datathon2022 -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "datathon2022", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "datathon2022", "YOUR_CONSOLE_SCRIPT"]
